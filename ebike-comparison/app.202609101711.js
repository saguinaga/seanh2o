(function () {
  ...
  const BRAKE_SCORES = { coaster: 0, rim: 4, mechanical_disc: 12, hydraulic_disc: 24, electronic_disc: 18 };
  const FEATURE_DEFS = {
  ...
  const skipListFilter = document.getElementById("skipListFilter");
  ...
  function hasFrontBrake(bike) {
    if (bike.brakes_front === true) return true;
    if (bike.brakes_front === false) return false;
    return ["mechanical_disc", "hydraulic_disc", "electronic_disc"].includes(bike.brake_type);
  }
  ...
  function getCardFilters() {
    return {
      ...
      showSkip: !!skipListFilter?.checked,
    };
  }
  ...
  function isCardVisible(card, f) {
    ...
    const skipOk = f.showSkip || card.dataset.deemphasize !== "yes";
    return tierOk && legalOk && priceOk && speedOk && weightOk && vehicleOk && skipOk;
  }
  ...
  function renderDetailPage(id) {
    ...
    const skipBadge = bike.deemphasize ? `<span class="badge danger">Skip list</span>` : "";
    const noteHtml = bike.catalog_note ? `<p class="detail-catalog-note">${bike.catalog_note}</p>` : "";
  ...
  function scatterLineTwo(bike, xFn, yFn, xLabel, yLabel) {
    const x = xFn(bike);
    const y = yFn(bike);
    const xBit =
      /price/i.test(xLabel) ? `${formatMoney(x)} landed` : `${xLabel} ${x}`;
    let yBit = `${yLabel} ${y}`;
    if (/safety/i.test(yLabel)) yBit = `Safety ${y}/100`;
    else if (/range/i.test(yLabel)) yBit = `PAS range ${y} mi`;
    else if (/wh\/mi/i.test(yLabel)) yBit = `${Number(y).toFixed(1)} Wh/mi PAS`;
    else if (/smart/i.test(yLabel)) yBit = `Smart ${y}`;
    return `${xBit}. ${yBit}.`;
  }
  ...
  function renderScatterChart(elId, bikes, xFn, yFn, xLabel, yLabel) {
    ...
    const dots = bikes
      .map((b) => {
        const cx = scaleX(xFn(b)).toFixed(1);
        const cy = scaleY(yFn(b)).toFixed(1);
        const name = escapeHtml(`${b.brand} ${b.model}`);
        const line2 = escapeHtml(scatterLineTwo(b, xFn, yFn, xLabel, yLabel));
        const href = `#/ride/${encodeURIComponent(b.id)}`;
        return `<a class="chart-dot-link" href="${href}" data-id="${escapeHtml(b.id)}" data-name="${name}" data-blurb="${line2}" aria-label="${name}. ${line2} Open details.">
          <circle class="chart-dot-hit" cx="${cx}" cy="${cy}" r="9"/>
          <circle class="chart-dot" cx="${cx}" cy="${cy}" r="5"/>
        </a>`;
      })
      .join("");
    el.innerHTML = `
      <svg viewBox="0 0 ${W} ${H}" class="chart-svg" role="img" aria-label="${escapeHtml(yLabel)} vs ${escapeHtml(xLabel)}. Hover a dot for the bike, click for details.">
        ...
        <text x="${W / 2}" y="${H - 4}" class="chart-label" text-anchor="middle">${escapeHtml(xLabel)}</text>
        <text x="10" y="${H / 2}" class="chart-label" transform="rotate(-90 10 ${H / 2})">${escapeHtml(yLabel)}</text>
        ${dots}
      </svg>
      <div class="chart-tip" hidden>
        <p class="chart-tip-name"></p>
        <p class="chart-tip-blurb"></p>
        <a class="chart-tip-link" href="#">View details</a>
      </div>`;
    bindScatterTips(el);
  }

  function bindScatterTips(el) {
    const tip = el.querySelector(".chart-tip");
    if (!tip) return;
    const nameEl = tip.querySelector(".chart-tip-name");
    const blurbEl = tip.querySelector(".chart-tip-blurb");
    const linkEl = tip.querySelector(".chart-tip-link");

    function hideTip() {
      tip.hidden = true;
      el.querySelectorAll(".chart-dot-link.is-hover").forEach((n) => n.classList.remove("is-hover"));
    }
    ...
    function showTip(link) {
      ...
      const href = `#/ride/${encodeURIComponent(id)}`;
      ...
      const dot = link.querySelector(".chart-dot") || link;
      const r = dot.getBoundingClientRect();
      const tx = r.left - wrap.left + r.width / 2;
      const ty = r.top - wrap.top;
      tip.style.left = `${tx}px`;
      tip.style.top = `${Math.max(8, ty)}px`;
      const tipR = tip.getBoundingClientRect();
      if (tipR.right > wrap.right - 6) {
        tip.style.left = `${Math.max(8, wrap.width - tipR.width - 8)}px`;
      }
      if (tipR.top < wrap.top + 4) {
        tip.style.top = `${r.bottom - wrap.top + 10}px`;
      }
    }

    el.querySelectorAll(".chart-dot-link").forEach((link) => {
      link.addEventListener("mouseenter", () => showTip(link));
      link.addEventListener("focus", () => showTip(link));
      link.addEventListener("click", (e) => {
        const id = link.getAttribute("data-id");
        if (!id || !bikeMap[id]) return;
        e.preventDefault();
        openDetail(id);
      });
    });
    ...
    linkEl.addEventListener("click", (e) => {
      const id = (linkEl.getAttribute("href") || "").replace(/^#\/ride\//, "");
      const decoded = decodeURIComponent(id);
      if (!bikeMap[decoded]) return;
      e.preventDefault();
      openDetail(decoded);
    });
  }
  ...
  [colorFilter, legalFilter, priceFilter, speedFilter, weightFilter, efficiencyFilter, vehicleFilter, smartFilter, skipListFilter].forEach((el) =>
