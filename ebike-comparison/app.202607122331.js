(function () {
  ...
  function refreshLegalForRiderAge(age) {
    Object.values(bikeMap).forEach((bike) => {
      bike.legal_for_age = evaluateBikeLegal(bike, age);
    });
    cards().forEach((card) => {
      const bike = bikeMap[card.dataset.id];
      if (bike) card.dataset.legal = bike.legal_for_age !== false ? "yes" : "no";
    });
    rows().forEach((row) => {
      const bike = bikeMap[row.dataset.id];
      if (bike) row.dataset.legal = bike.legal_for_age !== false ? "yes" : "no";
    });
  }
  ...
  function clearWizardResults() {
    config.riderAge = DEFAULT_RIDER_AGE;
    refreshLegalForRiderAge(DEFAULT_RIDER_AGE);
    clearWizardCardDecorations();
    if (wizardPicks) wizardPicks.hidden = true;
    if (wizardPicksGrid) wizardPicksGrid.innerHTML = "";
  }

  function applyWizardProfile(profile, { persist = true } = {}) {
    if (!profile?.completed) return;
    if (persist) saveWizardProfile(profile);

    config.riderAge = profile.age;
    refreshLegalForRiderAge(profile.age);
  ...
  function initWizard() {
    if (wizardProfile?.completed) {
      ...
      applyWizardProfile(wizardProfile, { persist: false });
