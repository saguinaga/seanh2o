(function () {
  ...
  function detailSpecBlock(title, bodyHtml) {
    const body = (bodyHtml || "").trim();
    if (!body) return "";
    return `<section class="detail-spec-block"><h3>${title}</h3><div class="detail-spec-body">${body}</div></section>`;
  }
  ...
  function featureBlockHtml(bike, { detail = false } = {}) {
    ...
    const groupRows = FEATURE_GROUPS.map(([gid, glabel]) => {
      ...
      const tags = (detail ? items : active)
        ...
        .join("");
      return `<li class="feature-group-row"><span class="spec-label">${glabel}</span><span class="feature-tags">${tags}</span></li>`;
    })
      .filter(Boolean)
      .join("");
    const scoreBadge =
  ...
  function batteryBlockHtml(bike, { detail = false } = {}) {
    ...
    const effLine =
      b.efficiency_label && b.efficiency_label !== "—"
        ? `<li><span class="spec-label">Efficiency</span> <span class="efficiency-tag efficiency-${b.efficiency_rating || "unknown"}">${b.efficiency_label}</span></li>`
        : "";
    const rows = `<ul class="battery-facts${detail ? " battery-facts--detail" : ""}">
        <li><span class="spec-label">Range</span> ${b.range_label}</li>
        <li><span class="spec-label">Capacity</span> ${b.capacity_label}</li>
        <li><span class="spec-label">Charging</span> ${b.charge_label}${note}</li>
        ${effLine}
      </ul>`;
