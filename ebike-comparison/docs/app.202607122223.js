(function () {
  ...
  function chatGreeting() {
    ...
    if (bike) {
      return `Hey — I'm ${CHAT_AGENT_NAME}. I can answer questions about the ${productLabel(bike)}, compare it to other rides, or explain safety and legal rules.\n\n${locNote}`;
    }
    return `Hey — I'm ${CHAT_AGENT_NAME}. Ask me about e-bikes and scooters — safety scores, battery range, weight, and what's legal for a ${config.riderAge || 12}-year-old.\n\n${locNote}`;
  }
  ...
  function answerChatLocal(query) {
    ...
    if (isLocationMessage(q)) return handleLocationMessage(q);

    if (isLegalQuestion(q) && !chatLocation?.confirmed) {
      return locationPromptMessage();
    }
    ...
    if (isLegalQuestion(q) || /(hb|huntington|beach|path|10\s*mph|sidewalk)/i.test(q)) {
      return buildLegalAnswer(chatLocation, { bike });
    }
    ...
    if (/scooter/.test(lower) && !bike) {
      ...
      return `${extra}\n\n${buildLegalAnswer(chatLocation, { bike })}`;
    }
    ...
    return `Try asking about safety scores, California helmet rules, Class 1/2/3, local path rules (${chatLocation?.label || "set your city"}), battery range, or open a product page for that specific ride.`;
  }
