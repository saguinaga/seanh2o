(function () {
  const STORAGE = {
    ...
    wizard: "ebike_wizard_v1",
    wizardPrompted: "ebike_wizard_prompted_v1",
    riderAge: "ebike_rider_age_v1",
  };
  ...
  const DEFAULT_RIDER_AGE = config.riderAge || 12;
  ...
  function weightPortability(lb) {
    ...
    if (w <= 40) return { tier: "easy", label: "Easy carry", hint: `A ${getRiderAge()}-year-old can usually lift and walk this alone.` };
  ...
  function getRiderAge() {
    return config.riderAge || DEFAULT_RIDER_AGE;
  }

  function loadStoredRiderAge() {
    try {
      const n = parseInt(localStorage.getItem(STORAGE.riderAge), 10);
      if (n >= 8 && n <= 99) return n;
    } catch (_) {}
    return null;
  }

  function saveRiderAge(age) {
    localStorage.setItem(STORAGE.riderAge, String(age));
  }
  ...
  function buildProductFaq(bike) {
    ...
    if (bike.legal_for_age === false) {
      items.push({
        q: `Can a ${age}-year-old legally ride the ${name}?`,
        a: isScooter
          ? `This model is flagged as not appropriate for a ${age}-year-old under our Huntington Beach rules checklist. Review local scooter path limits and helmet requirements before riding.`
          : bike.e_bike_class === 3
            ...
            : `Our checklist flags legal concerns for a ${age}-year-old on this model. Expand Legal notes below and verify current California and HB rules.`,
      });
    } else {
      const classNote =
        !isScooter && bike.e_bike_class
          ...
          : "";
      items.push({
        q: `Can a ${age}-year-old ride the ${name}?`,
    ...
    if (bike.weight_lb != null) {
      ...
      items.push({
        q: `Can a ${age}-year-old carry the ${name}?`,
  ...
  function handleLocationMessage(q) {
    ...
    if (loc.hasLocalRules) {
      return `Got it — I'll use ${loc.label} for local rules. Ask about helmets, path speed limits, Class 3, or what's legal for a ${getRiderAge()}-year-old.`;
    }
  ...
  function chatGreeting() {
    ...
    return `Hey — I'm ${CHAT_AGENT_NAME}. I search our ride catalog locally — try "lightest legal option", "compare Lectric and Rad", or what's safe for a ${getRiderAge()}-year-old.\n\n${locNote}`;
  }
  ...
  function renderChatSuggestions() {
    ...
    const suggestions = bike
      ? [
          ...
          `Is the ${bike.brand} ${bike.model} legal for a ${getRiderAge()}-year-old here?`,
  ...
  function applyCardFilters() {
    ...
    if (wizardProfile?.completed) {
      highlightWizardPicks(wizardProfile);
      reorderCardsByWizardScore(wizardProfile);
    } else {
      clearWizardCardDecorations();
    }
    updateHiddenUi();
    renderCharts();
  }
  ...
  // ── Needs wizard ──
  const WIZARD_USE_PRESETS = {
    school: { safety: 5, price: 4, speed: 1, weight: 4, range: 3 },
    commute: { safety: 4, price: 3, speed: 3, weight: 3, range: 4 },
    errands: { safety: 3, price: 4, speed: 2, weight: 5, range: 2 },
    fun: { safety: 3, price: 2, speed: 5, weight: 2, range: 3 },
  };
  const WIZARD_USE_LABELS = {
    school: "school / kid commute",
    commute: "daily commute",
    errands: "errands & short trips",
    fun: "fun & exploring",
  };

  let wizardProfile = loadWizardProfile();
  let wizardStep = 0;
  const wizardModal = document.getElementById("wizardModal");
  const wizardPicks = document.getElementById("wizardPicks");
  const wizardPicksGrid = document.getElementById("wizardPicksGrid");
  const wizardPicksNote = document.getElementById("wizardPicksNote");

  function loadWizardProfile() {
    try {
      const raw = localStorage.getItem(STORAGE.wizard);
      if (raw) return JSON.parse(raw);
    } catch (_) {}
    return null;
  }
  ...
  function saveWizardProfile(profile) {
    ...
    if (profile) localStorage.setItem(STORAGE.wizard, JSON.stringify(profile));
    else localStorage.removeItem(STORAGE.wizard);
  }

  function markWizardPrompted() {
    localStorage.setItem(STORAGE.wizardPrompted, "1");
  }

  function evaluateBikeLegal(bike, age = 12) {
    if (bike.vehicle_type === "scooter") return true;
    const cls = bike.e_bike_class;
    if ((cls === 3 || cls === "3") && age < 16) return false;
    return true;
  }

  function legalBadgeCompactHtml(bike) {
    if (bike.legal_for_age !== false) {
      return `<span class="badge ok card-legal-badge">${bike.vehicle_type === "scooter" ? "OK" : "Legal"}</span>`;
    }
    return `<span class="badge danger card-legal-badge" title="Not legal for age ${getRiderAge()} in CA">✗</span>`;
  }

  function legalBadgeDetailHtml(bike) {
    const age = getRiderAge();
    if (bike.legal_for_age !== false) {
      return `<span class="badge ok">${bike.vehicle_type === "scooter" ? "OK w/ rules" : "Legal"}</span>`;
    }
    return `<span class="badge danger">Not legal age ${age}</span>`;
  }

  function legalBadgeCompareHtml(bike) {
    const age = getRiderAge();
    return bike.legal_for_age !== false
      ? `<span class="badge ok">Legal age ${age}</span>`
      : `<span class="badge danger">Illegal age ${age}</span>`;
  }

  function updateLegalAgeLabels(age) {
    const legalAge = document.getElementById("legalFilterAge");
    if (legalAge) legalAge.textContent = String(age);
    const tableHeader = document.getElementById("tableLegalHeader");
    if (tableHeader) tableHeader.textContent = `Legal (age ${age})`;
  }

  function refreshLegalUiForAge(age) {
    Object.values(bikeMap).forEach((bike) => {
      bike.legal_for_age = evaluateBikeLegal(bike, age);
    });
    cards().forEach((card) => {
      const bike = bikeMap[card.dataset.id];
      if (!bike) return;
      card.dataset.legal = bike.legal_for_age !== false ? "yes" : "no";
      const badge = card.querySelector(".card-legal-badge");
      if (badge) badge.outerHTML = legalBadgeCompactHtml(bike);
    });
    rows().forEach((row) => {
      const bike = bikeMap[row.dataset.id];
      if (!bike) return;
      row.dataset.legal = bike.legal_for_age !== false ? "yes" : "no";
      const cell = row.querySelector(".table-legal-cell");
      if (cell) cell.textContent = bike.legal_for_age !== false ? "Yes" : "No";
    });
    updateLegalAgeLabels(age);
  }

  function setRiderAge(age, { applyFilters = true } = {}) {
    const n = Math.min(99, Math.max(8, parseInt(age, 10) || DEFAULT_RIDER_AGE));
    config.riderAge = n;
    saveRiderAge(n);
    const input = document.getElementById("riderAgeInput");
    if (input && parseInt(input.value, 10) !== n) input.value = String(n);
    const wizardAge = document.getElementById("wizardAge");
    if (wizardAge && parseInt(wizardAge.value, 10) !== n) wizardAge.value = String(n);
    refreshLegalUiForAge(n);
    if (applyFilters) {
      applyCardFilters();
      applyTableFilters();
      updateComparePanel();
  ...
  function initRiderAge() {
    const age = loadStoredRiderAge() ?? (wizardProfile?.completed ? wizardProfile.age : null) ?? DEFAULT_RIDER_AGE;
    setRiderAge(age, { applyFilters: false });
  }

  function readWizardForm() {
    const age = parseInt(document.getElementById("wizardAge")?.value, 10) || DEFAULT_RIDER_AGE;
    const heightIn = parseInt(document.getElementById("wizardHeight")?.value, 10) || 62;
    const city = (document.getElementById("wizardCity")?.value || config.defaultCity || "Huntington Beach").trim();
    return {
      completed: true,
      age,
      heightIn,
      city,
      vehicle: document.getElementById("wizardVehicle")?.value || "all",
      use: document.getElementById("wizardUse")?.value || "commute",
      budget: document.getElementById("wizardBudget")?.value || "all",
      priorities: {
        safety: parseInt(document.getElementById("wizardPriSafety")?.value, 10) || 3,
        price: parseInt(document.getElementById("wizardPriPrice")?.value, 10) || 3,
        speed: parseInt(document.getElementById("wizardPriSpeed")?.value, 10) || 3,
        weight: parseInt(document.getElementById("wizardPriWeight")?.value, 10) || 3,
        range: parseInt(document.getElementById("wizardPriRange")?.value, 10) || 3,
      },
    };
  }

  function fillWizardForm(profile) {
    if (!profile) return;
    const set = (id, val) => {
      const el = document.getElementById(id);
      if (el && val != null) el.value = val;
    };
    set("wizardAge", profile.age);
    set("wizardHeight", profile.heightIn);
    set("wizardCity", profile.city);
    set("wizardVehicle", profile.vehicle);
    set("wizardUse", profile.use);
    set("wizardBudget", profile.budget);
    const p = profile.priorities || {};
    set("wizardPriSafety", p.safety);
    set("wizardPriPrice", p.price);
    set("wizardPriSpeed", p.speed);
    set("wizardPriWeight", p.weight);
    set("wizardPriRange", p.range);
  }

  function applyUsePresetToSliders(use) {
    const preset = WIZARD_USE_PRESETS[use];
    if (!preset) return;
    Object.entries(preset).forEach(([key, val]) => {
      const el = document.getElementById(`wizardPri${key.charAt(0).toUpperCase()}${key.slice(1)}`);
      if (el) el.value = val;
    });
  }
  ...
  function setWizardStep(step) {
    ...
    document.querySelectorAll(".wizard-panel").forEach((panel) => {
      const n = parseInt(panel.dataset.wizardStep, 10);
      panel.hidden = n !== wizardStep;
    });
    ...
    const done = document.getElementById("btnWizardDone");
    if (back) back.hidden = wizardStep === 0;
    if (next) next.hidden = wizardStep === 2;
    if (done) done.hidden = wizardStep !== 2;
  }

  function openWizardModal(step = 0) {
    if (!wizardModal) return;
    if (wizardProfile?.completed) fillWizardForm(wizardProfile);
    else fillWizardForm({
      age: DEFAULT_RIDER_AGE,
      heightIn: 62,
      city: config.defaultCity || "Huntington Beach",
      vehicle: "all",
      use: "commute",
      budget: "all",
      priorities: WIZARD_USE_PRESETS.commute,
    });
    setWizardStep(step);
    markWizardPrompted();
  ...
  function skipWizard() {
    saveWizardProfile(null);
    clearWizardResults();
    closeWizardModal();
    markWizardPrompted();
  }

  function scoreBikeForWizard(bike, profile) {
    const p = profile.priorities || {};
    let total = 0;
    let weight = 0;
    const add = (priority, normalized) => {
      const w = priority || 3;
      total += w * Math.max(0, Math.min(1, normalized));
      weight += w;
    };

    add(p.safety, (bike.safety_score || 0) / 100);

    const maxPrice = profile.budget === "all" ? 2500 : parseFloat(profile.budget);
    const price = bikePrice(bike) || maxPrice;
    add(p.price, maxPrice > 0 ? 1 - Math.min(1, price / maxPrice) : 0.5);

    add(p.speed, Math.min(1, (bike.max_speed_mph || 0) / 28));

    const lbs = bike.weight_lb ?? 60;
    add(p.weight, Math.max(0, 1 - (lbs - 25) / 55));

    const rangeMi = bike.battery_display?.range_miles || bike.battery_range_miles_pas || 0;
    add(p.range, Math.min(1, rangeMi / 35));

    let bonus = 0;
    if (profile.use === "school") {
      if (bike.legal_for_age !== false) bonus += 0.15;
      if ((bike.max_speed_mph || 99) <= 20) bonus += 0.08;
      if (lbs <= 55) bonus += 0.05;
    } else if (profile.use === "commute") {
      if (rangeMi >= 20) bonus += 0.08;
      if (bike.feature_display?.is_smart) bonus += 0.04;
    } else if (profile.use === "errands") {
      if (lbs <= 50) bonus += 0.1;
    } else if (profile.use === "fun") {
      if ((bike.max_speed_mph || 0) >= 20) bonus += 0.08;
      if ((bike.feature_display?.luxury_score || 0) >= 20) bonus += 0.05;
    }

    const height = profile.heightIn || 62;
    if (height < 60 && lbs <= 45) bonus += 0.06;

    return weight > 0 ? total / weight + bonus : bonus;
  }

  function getWizardTopPicks(profile, limit = 3) {
    const f = getCardFilters();
    return [...cards()]
      .filter((card) => isCardVisible(card, f))
      .map((card) => bikeMap[card.dataset.id])
      .filter(Boolean)
      .map((bike) => ({ bike, score: scoreBikeForWizard(bike, profile) }))
      .sort((a, b) => b.score - a.score)
      .slice(0, limit);
  }

  function wizardPrioritySummary(profile) {
    const p = profile.priorities || {};
    const ranked = Object.entries(p)
      .sort((a, b) => b[1] - a[1])
      .filter(([, v]) => v >= 4)
      .map(([k]) => ({ safety: "safety", price: "price", speed: "speed", weight: "light weight", range: "range" }[k]))
      .filter(Boolean);
    return ranked.length ? ranked.join(" & ") : "balanced priorities";
  }

  function renderWizardPicks(profile) {
    if (!wizardPicks || !wizardPicksGrid) return;
    const picks = getWizardTopPicks(profile);
    if (!picks.length) {
      wizardPicks.hidden = true;
      return;
    }
    wizardPicks.hidden = false;
    const loc = profile.city ? `${profile.city}, CA` : "your area";
    if (wizardPicksNote) {
      wizardPicksNote.textContent = `Age ${profile.age} · ${loc} · ${WIZARD_USE_LABELS[profile.use] || profile.use} · ${wizardPrioritySummary(profile)}`;
    }
    wizardPicksGrid.innerHTML = picks
      .map(({ bike }, i) => {
        const gallery = productGallery(bike);
        const img = gallery[0]
          ? `<img src="${gallery[0]}" alt="" loading="lazy">`
          : `<span class="placeholder-icon">${bike.vehicle_type === "scooter" ? "🛴" : "🚲"}</span>`;
        return `<article class="wizard-pick-card">
          <span class="wizard-pick-rank">#${i + 1}</span>
          <button type="button" class="wizard-pick-open card-open" data-id="${bike.id}">
            <div class="wizard-pick-thumb">${vehicleMarkHtml(bike)}${img}</div>
            <div class="wizard-pick-body">
              <strong>${bike.brand} ${bike.model}</strong>
              <span>${formatMoney(bikePrice(bike))} · Safety ${bike.safety_score} · ${formatWeightLb(bike.weight_lb)}</span>
            </div>
          </button>
        </article>`;
      })
  ...
  function highlightWizardPicks(profile) {
    const topIds = new Set(getWizardTopPicks(profile).map(({ bike }) => bike.id));
    cards().forEach((card) => {
      card.classList.toggle("wizard-pick-highlight", topIds.has(card.dataset.id));
    });
  }

  function reorderCardsByWizardScore(profile) {
    if (!cardGrid) return;
    const f = getCardFilters();
    const scored = [...cards()]
      .filter((card) => isCardVisible(card, f))
      .map((card) => ({
        card,
        score: scoreBikeForWizard(bikeMap[card.dataset.id], profile),
      }))
      .sort((a, b) => b.score - a.score);
    scored.forEach(({ card }) => cardGrid.appendChild(card));
  }

  function clearWizardCardDecorations() {
    cards().forEach((card) => card.classList.remove("wizard-pick-highlight"));
  }

  function clearWizardResults() {
    setRiderAge(DEFAULT_RIDER_AGE, { applyFilters: false });
    clearWizardCardDecorations();
    if (wizardPicks) wizardPicks.hidden = true;
    if (wizardPicksGrid) wizardPicksGrid.innerHTML = "";
  }

  function applyWizardProfile(profile, { persist = true, syncAge = true } = {}) {
    if (!profile?.completed) return;
    if (persist) saveWizardProfile(profile);

    if (syncAge) setRiderAge(profile.age, { applyFilters: false });

    const loc = resolveChatLocation(profile.city) || {
      id: null,
      label: `${profile.city}, CA`,
      hasLocalRules: false,
      confirmed: true,
    };
    saveChatLocation(loc);

    if (vehicleFilter) vehicleFilter.value = profile.vehicle;
    if (priceFilter) priceFilter.value = profile.budget;

    if (profile.age < 16 || profile.use === "school") {
      if (legalFilter) legalFilter.value = "yes";
    }

    if (profile.use === "school") {
      if (weightFilter) weightFilter.value = "55";
      if (speedFilter) speedFilter.value = "20";
    } else if (profile.use === "errands" && (profile.priorities?.weight || 0) >= 4) {
      if (weightFilter) weightFilter.value = "40";
    } else if (profile.use === "fun" && (profile.priorities?.speed || 0) >= 4) {
      if (speedFilter) speedFilter.value = "all";
    }

    if (efficiencyFilter) efficiencyFilter.checked = (profile.priorities?.range || 0) >= 4;
    if (smartFilter) smartFilter.checked = (profile.priorities?.safety || 0) >= 4;

    applyCardFilters();
    applyTableFilters();
    renderWizardPicks(profile);
  }

  function finishWizard() {
    const profile = readWizardForm();
    applyWizardProfile(profile);
    closeWizardModal();
    document.getElementById("browseView")?.scrollIntoView({ behavior: "smooth", block: "start" });
  }

  function initWizard() {
    if (wizardProfile?.completed) {
      fillWizardForm(wizardProfile);
      applyWizardProfile(wizardProfile, { persist: false, syncAge: false });
    } else if (!localStorage.getItem(STORAGE.wizardPrompted)) {
      window.setTimeout(() => openWizardModal(0), 900);
    }
  }
  ...
  function compactCardInner(bike) {
    ...
    const legalBadge = legalBadgeCompactHtml(bike);
    const smartBadge =
  ...
  function renderDetailPage(id) {
    ...
    const legalBadge = legalBadgeDetailHtml(bike);
  ...
  function ensureChartTooltip() {
    if (!chartTipEl) {
      ...
      chartTipEl.id = "chartTooltip";
      chartTipEl.className = "chart-tooltip";
  ...
  function formatChartAxis(label, value) {
    if (value == null || Number.isNaN(value)) return "—";
    if (label === "Price") return formatMoney(value);
    if (label === "Safety") return `${Math.round(value)}/100`;
    if (label === "Max mph") return `${value} mph`;
    if (label === "PAS range (mi)") return `${Math.round(value * 10) / 10} mi`;
    if (label === "Wh/mi PAS") return `${Number(value).toFixed(1)} Wh/mi`;
    if (label === "Smart score") return String(Math.round(value));
    return String(value);
  }

  function formatChartTick(label, value) {
    if (label === "Price") {
    ...
    if (label === "Safety" || label === "Smart score") return String(Math.round(value));
    if (label === "Max mph") return `${value}`;
    if (label === "PAS range (mi)") return `${Math.round(value)}`;
    if (label === "Wh/mi PAS") return `${Number(value).toFixed(0)}`;
    return formatChartAxis(label, value);
  }
  ...
  function hideChartTooltip() {
    ...
    document
      .querySelectorAll(".chart-dot--active, .bar-row--active")
      .forEach((el) => el.classList.remove("chart-dot--active", "bar-row--active"));
  }

  function scatterTooltipHtml(bike, xLabel, yLabel, xVal, yVal) {
    const extras = [];
    if (xLabel !== "Price" && yLabel !== "Price") extras.push(["Price", formatMoney(bikePrice(bike))]);
    if (xLabel !== "Safety" && yLabel !== "Safety") extras.push(["Safety", `${bike.safety_score}/100`]);
    extras.push(["Max speed", `${bike.max_speed_mph ?? "—"} mph`]);
    extras.push(["Weight", formatWeightLb(bike.weight_lb)]);
    const bd = bike.battery_display || {};
    if (bd.range_label && bd.range_label !== "—") extras.push(["Range", bd.range_label]);
    extras.push(["Tier", bike.tier_label || tierLabels[bike.tier] || bike.tier]);

    const rows = [
      [xLabel, formatChartAxis(xLabel, xVal)],
      [yLabel, formatChartAxis(yLabel, yVal)],
      ...extras,
    ];

    return `<div class="chart-tooltip-title">${bike.brand} ${bike.model}</div>
      <dl class="chart-tooltip-metrics">${rows
        .map(([k, v]) => `<div><dt>${k}</dt><dd>${v}</dd></div>`)
        .join("")}</dl>`;
  }

  function tierBarTooltipHtml(tier, bikes, valueFn, valueLabel, fmt) {
    const group = bikes
      .filter((b) => b.tier === tier)
      .sort((a, b) => (valueFn(b) ?? 0) - (valueFn(a) ?? 0));
    const label = tierLabels[tier] || tier;
    const avg = group.length ? group.reduce((s, b) => s + (valueFn(b) ?? 0), 0) / group.length : 0;
    const rows = group
      .map(
        (b) =>
          `<div class="chart-tooltip-item"><span class="chart-tooltip-item-name">${b.brand} ${b.model}</span><span class="chart-tooltip-item-val">${fmt(valueFn(b))} · ${formatMoney(bikePrice(b))}</span></div>`
      )
      .join("");
    return `<div class="chart-tooltip-title">${label} · ${group.length} bikes</div>
      <div class="chart-tooltip-sub">Avg ${valueLabel}: ${fmt(avg)}</div>
      <div class="chart-tooltip-list">${rows}</div>`;
  }

  function checklistBarTooltipHtml(key, keyLabel, bikes) {
    const pass = bikes.filter((b) => (b.safety_checklist || []).find((i) => i.key === key)?.ok);
    const fail = bikes.filter((b) => !(b.safety_checklist || []).find((i) => i.key === key)?.ok);
    const passList = pass
      .slice(0, 8)
      .map(
        (b) =>
          `<div class="chart-tooltip-item chart-tooltip-item--ok"><span>${b.brand} ${b.model}</span><span>${formatMoney(bikePrice(b))} · ${b.safety_score}/100</span></div>`
      )
      .join("");
    const failList = fail
      .slice(0, 6)
      .map(
        (b) =>
          `<div class="chart-tooltip-item chart-tooltip-item--no"><span>${b.brand} ${b.model}</span><span>${formatMoney(bikePrice(b))}</span></div>`
      )
      .join("");
    const hidden = pass.length + fail.length - 14;
    const more = hidden > 0 ? `<div class="chart-tooltip-more">+${hidden} more in filter</div>` : "";
    return `<div class="chart-tooltip-title">${keyLabel}</div>
      <div class="chart-tooltip-sub">${pass.length}/${bikes.length} pass (${Math.round((pass.length / bikes.length) * 100)}%)</div>
      ${pass.length ? `<div class="chart-tooltip-section">Passes</div><div class="chart-tooltip-list">${passList}</div>` : ""}
      ${fail.length ? `<div class="chart-tooltip-section">Missing</div><div class="chart-tooltip-list">${failList}</div>` : ""}
      ${more}`;
  }

  function wireScatterTooltips(container, bikes, xFn, yFn, xLabel, yLabel) {
    container.querySelectorAll(".chart-dot[data-id]").forEach((dot) => {
      const bike = bikeMap[dot.dataset.id];
      if (!bike) return;
      const html = scatterTooltipHtml(bike, xLabel, yLabel, xFn(bike), yFn(bike));
      const show = (e) => {
      ...
      dot.addEventListener("mousemove", (e) => {
        if (!dot.classList.contains("chart-dot--active")) return;
  ...
  function wireTierBars(el, bikes, valueFn, valueLabel, fmt) {
    el.querySelectorAll(".bar-row[data-tier]").forEach((row) => {
      const tier = row.dataset.tier;
      const show = (e) => {
        row.classList.add("bar-row--active");
        showChartTooltip(tierBarTooltipHtml(tier, bikes, valueFn, valueLabel, fmt), e.clientX, e.clientY);
      };
      row.addEventListener("mouseenter", show);
      row.addEventListener("focus", show);
      row.addEventListener("mousemove", (e) => {
        if (!row.classList.contains("bar-row--active")) return;
        positionChartTooltip(e.clientX, e.clientY);
      });
  ...
  function wireChecklistBars(el, bikes) {
    const keys = [
    ...
    el.querySelectorAll(".bar-row[data-checklist]").forEach((row) => {
      const meta = keys.find((k) => k.key === row.dataset.checklist);
      if (!meta) return;
      const show = (e) => {
        row.classList.add("bar-row--active");
        showChartTooltip(checklistBarTooltipHtml(meta.key, meta.label, bikes), e.clientX, e.clientY);
      };
      row.addEventListener("mouseenter", show);
      row.addEventListener("focus", show);
      row.addEventListener("mousemove", (e) => {
  ...
  function scatterAxisTicks(minX, maxX, minY, maxY, xLabel, yLabel, scaleX, scaleY, W, H, pad) {
    const xVals = [minX, (minX + maxX) / 2, maxX];
    const yVals = [minY, maxY];
    const xTicks = xVals
      .map((v) => {
        const x = scaleX(v);
        return `<text x="${x}" y="${H - 14}" class="chart-tick" text-anchor="middle">${formatChartTick(xLabel, v)}</text>`;
      })
      .join("");
    const yTicks = yVals
      .map((v) => {
        const y = scaleY(v);
        return `<text x="${pad.l - 6}" y="${y + 3}" class="chart-tick" text-anchor="end">${formatChartTick(yLabel, v)}</text>`;
      })
      .join("");
  ...
  function renderEfficiencyTierChart(elId, bikes) {
    ...
    el.innerHTML = data
      .map(
        (d) => `
      <div class="bar-row bar-row--tip" data-tier="${d.tier}" tabindex="0" role="button" aria-label="${d.label} average ${d.avg.toFixed(1)} Wh per mile">
  ...
  function renderScatterChart(elId, bikes, xFn, yFn, xLabel, yLabel) {
    ...
    const ticks = scatterAxisTicks(minX, maxX, minY, maxY, xLabel, yLabel, scaleX, scaleY, W, H, pad);
    const dots = bikes
      .map((b) => {
        const cx = scaleX(xFn(b));
        const cy = scaleY(yFn(b));
        const tip = `${b.brand} ${b.model}`;
        return `<circle cx="${cx}" cy="${cy}" r="7" class="chart-dot" data-id="${b.id}" tabindex="0" role="button" aria-label="${tip.replace(/"/g, "'")}"></circle>`;
      })
      .join("");
    el.innerHTML = `
    ...
    wireScatterTooltips(el, bikes, xFn, yFn, xLabel, yLabel);
  }
  ...
  function renderTierChart(elId, bikes) {
    ...
    el.innerHTML = data
      .map(
        (d) => `
      <div class="bar-row bar-row--tip" data-tier="${d.tier}" tabindex="0" role="button" aria-label="${d.label} average safety ${Math.round(d.avg)}">
        <span class="bar-label">${d.label}</span>
      ...
      .join("");
    wireTierBars(el, bikes, (b) => b.safety_score, "safety", (v) => `${Math.round(v)}/100`);
  }
  ...
  function renderChecklistChart(elId, bikes) {
    ...
    el.innerHTML = keys
      .map((k) => {
        ...
        return `
        <div class="bar-row bar-row--tip" data-checklist="${k.key}" tabindex="0" role="button" aria-label="${k.label} ${pct} percent pass rate">
  ...
  function compareShortModel(bike) {
    const raw = bike.model || "";
    return raw.length > 22 ? `${raw.slice(0, 20)}…` : raw;
  }

  function compareColumnHeader(bike) {
    return `<span class="compare-th-brand">${bike.brand}</span><span class="compare-th-model">${compareShortModel(bike)}</span>`;
  }

  function renderCompareColumn(colEl, bike, slotIndex) {
    if (!colEl) return false;
    if (!bike) {
      colEl.className = "compare-column compare-column--empty";
      colEl.removeAttribute("data-id");
      colEl.innerHTML = `
        <div class="compare-column-empty-inner">
          <span class="compare-column-slot-num" aria-hidden="true">${slotIndex + 1}</span>
          <p class="compare-column-empty-label">Empty slot</p>
          <p class="compare-column-empty-hint">Tap <strong>+ Compare</strong> on a ride</p>
        </div>`;
      return true;
    }

    colEl.className = "compare-column";
    colEl.dataset.id = bike.id;
    const gallery = bike.image_gallery?.length ? bike.image_gallery : bike.image_src ? [bike.image_src] : [];
    const mainImg = gallery[0];
    const img = mainImg
      ? `<img class="compare-column-img" src="${mainImg}" alt="">`
      : `<span class="compare-column-placeholder">${bike.vehicle_type === "scooter" ? "🛴" : "🚲"}</span>`;
    const legalBadge = legalBadgeCompareHtml(bike);
    const kind = bike.vehicle_type === "scooter" ? "Scooter" : `Class ${bike.e_bike_class || "?"}`;
    const bat = formatBattery(bike);
    const rangeShort = bat.range_label && bat.range_label !== "—" ? bat.range_label.split("·")[0].trim() : "—";

    colEl.innerHTML = `
      <button type="button" class="compare-column-remove" data-id="${bike.id}" aria-label="Remove ${bike.brand} ${bike.model} from compare">✕</button>
      <button type="button" class="compare-column-open card-open" data-id="${bike.id}">
        <div class="compare-column-thumb">${vehicleMarkHtml(bike)}${img}</div>
        <div class="compare-column-body">
          <strong class="compare-column-name">${bike.brand}</strong>
          <span class="compare-column-model">${compareShortModel(bike)}</span>
          <div class="compare-column-price">${formatMoney(bike.price)}</div>
          <div class="compare-column-stats">
            <span class="compare-stat compare-stat--safety">Safety <strong>${bike.safety_score}</strong></span>
            <span class="compare-stat">${formatWeightLb(bike.weight_lb)}</span>
            <span class="compare-stat">${bike.max_speed_mph} mph</span>
          </div>
          <div class="compare-column-meta">${legalBadge} · ${kind}</div>
          <div class="compare-column-meta compare-column-meta--sub">${brakeHtml(bike)} · ${rangeShort}</div>
        </div>
      </button>`;
    return true;
  }

  function renderCompareQuickPicks(selected) {
    const el = document.getElementById("compareQuickPicks");
    if (!el) return;
    ...
    const safest = selected.reduce((a, b) => ((a.safety_score || 0) > (b.safety_score || 0) ? a : b));
    const cheapest = selected.reduce((a, b) => (bikePrice(a) < bikePrice(b) ? a : b));
    const weighed = selected.filter((b) => b.weight_lb != null);
    const lightest = weighed.length
      ? weighed.reduce((a, b) => (a.weight_lb < b.weight_lb ? a : b))
      : null;
    const chips = [
      `<span class="compare-chip compare-chip--safety">🛡️ Safest <strong>${safest.brand}</strong> <em>${safest.safety_score}/100</em></span>`,
      `<span class="compare-chip compare-chip--price">💰 Cheapest <strong>${cheapest.brand}</strong> <em>${formatMoney(bikePrice(cheapest))}</em></span>`,
    ];
    if (lightest) {
      chips.push(
        `<span class="compare-chip compare-chip--weight">⚖️ Lightest <strong>${lightest.brand}</strong> <em>${formatWeightLb(lightest.weight_lb)}</em></span>`
      );
    }
  ...
  function toggleCompare(id) {
    ...
    if (wasIncluded) {
      ...
      compareIds.push(id);
      if (compareIds.length >= 2) openCompareDrawer();
    } else {
      compareIds.shift();
      compareIds.push(id);
      openCompareDrawer();
  ...
  function renderSafetyMatrix(selected) {
    ...
    const headers = selected.map((b) => `<th>${compareColumnHeader(b)}</th>`).join("");
  ...
  function updateComparePanel() {
    ...
    const countEl = document.getElementById("compareDrawerCount");
    ...
    if (slotsContainer) {
      ...
      for (let i = 0; i < MAX_COMPARE; i += 1) {
        ...
        renderCompareColumn(col, selected[i] || null, i);
        slotsContainer.appendChild(col);
      }
    }

    if (countEl) countEl.textContent = `${selected.length}/${MAX_COMPARE}`;
    if (clearBtn) clearBtn.hidden = selected.length === 0;

    if (hint) {
      hint.hidden = false;
      if (selected.length === 0) {
        hint.textContent = "Tap + Compare on any card (up to 3). Add 2+ to unlock the spec table.";
      } else if (selected.length === 1) {
        hint.textContent = "Add one more ride to unlock side-by-side specs and safety highlights.";
      } else if (selected.length < MAX_COMPARE) {
        hint.textContent = `${selected.length} of ${MAX_COMPARE} selected — add another, or scroll the table below.`;
      } else {
        hint.textContent = "Full queue — removing one lets you swap in a different ride.";
      }
    }

    renderCompareQuickPicks(selected);

    const hasCompare = compareIds.length > 0;
    if (compareFab) {
      compareFab.hidden = !hasCompare;
      compareFab.classList.toggle("compare-fab--ready", selected.length >= 2);
    }
  ...
  function resetPreferences() {
    if (!confirm("Reset hidden bikes, custom bikes, baseline choice, and ride finder answers?")) return;
    hiddenIds = [];
    customBikes = [];
    ...
    localStorage.removeItem(STORAGE.wizard);
    localStorage.removeItem(STORAGE.riderAge);
    wizardProfile = null;
    clearWizardResults();
  ...
  document.getElementById("riderAgeInput")?.addEventListener("change", (e) => setRiderAge(e.target.value));
  document.getElementById("riderAgeInput")?.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      e.preventDefault();
  ...
  document.addEventListener("click", (e) => {
    ...
    const removeCompare = e.target.closest(".compare-column-remove, .compare-slot-remove");
  ...
  document.getElementById("btnWizard")?.addEventListener("click", () => openWizardModal(0));
  document.getElementById("btnWizardEdit")?.addEventListener("click", () => openWizardModal(0));
  document.getElementById("btnWizardSkip")?.addEventListener("click", skipWizard);
  document.getElementById("btnWizardBack")?.addEventListener("click", () => setWizardStep(wizardStep - 1));
  document.getElementById("btnWizardNext")?.addEventListener("click", () => {
    if (wizardStep === 1) applyUsePresetToSliders(document.getElementById("wizardUse")?.value || "commute");
  ...
  document.getElementById("wizardUse")?.addEventListener("change", (e) => {
    applyUsePresetToSliders(e.target.value);
  });
